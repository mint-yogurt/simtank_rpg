...
**Current state:** the flat flag dict this sketch assumes now exists for real — `engine/game_state.py`'s `GameState.flags` — but the general `if`/`else` condition evaluation and action-list execution (the Event/Trigger System these two sketches describe) is still not built; nothing resolves a script ID into a set of actions yet. What *is* built on top of `GameState` is one concrete, hardcoded case of "trigger": containers. Every persistent object (container, sign, npc, warp — anything `data/maps/populate_yamls.py` stubs) gets a flag key for free, `engine.game_state.persistent_id(map_name, object_name)` — no registry file, no per-object declaration; the id is derived the same way every time, so it's usable the instant an object exists on a map. A `container`-type object's `contents` field (an item id from `data/items/items.yaml`, or `null` for pure flavor) is the "script": opening one with `contents` set adds the item to the shared `Inventory`, sets its flag, and appends a synthesized `"Received {item name}."` page after its authored `dialogue` — opening it again is inert (same as not being interactable) once the flag's set. A `contents: null` container just replays its dialogue forever, exactly like a sign, and never touches a flag. See `engine.input._open_container` for the actual logic. Conditional dialogue for signs/NPCs (picking a page list based on flag state) still isn't wired to anything — `GameState` can already answer "is this flag set," but nothing reads it for that yet.
...
- [x] Global game state / flags (`engine/game_state.py`'s `GameState`) — the flat flag dict + freeform `variables` dict the Event System sketch below assumes, real and save-able, but with no condition-evaluation engine built on top of it yet. `persistent_id(map_name, object_name)` gives every object on every map a stable flag key with nothing to declare or sync — see Items, NPCs, Dialogue, Events above
- [x] Container → inventory wiring — `engine.input._open_container`: a container with `contents` set (an item id) grants it to the shared `Inventory` and sets its flag the first time it's opened, appending a `"Received {name}."` page; inert (same as non-interactable) once opened. A `contents: null` container behaves like a sign — always replays, never sets a flag. This is a narrow, hardcoded case of "trigger," not the general Trigger System below
- [ ] Event System — condition evaluation (`if`/`else` chains) over `GameState.flags` → dialogue ID or action list. The flags themselves already exist (see above) — this is the "resolve a script ID into a decision" piece, still unbuilt
...
- [ ] Trigger System — executes a referenced script YAML on activation (doors, switches, more elaborate chest logic than a flat item grant) — distinct from `Warp` (above, already wired, no conditional behavior) and from the container item-grant wiring (above, a fixed narrow case, not a general script)
...
- New container loot → set that container's `contents:` (in its map's `obj_<map>.yaml`, hand-filled after `populate_yamls.py` stubs it to `null`) to an item id from `data/items/items.yaml`. Leave it `null` for a container that's dialogue-only, no loot.
